...
async function run() {
  const q = query(collection(db, "cloud_notes"), orderBy("createdAt", "desc"), limit(5));
  const snap = await getDocs(q);
  console.log("Documents found:", snap.size);
  snap.forEach(doc => {
    console.log(doc.id, "Title:", doc.data().title, "isStaged:", doc.data().isStaged, "Content length:", doc.data().content?.length);
  });
}

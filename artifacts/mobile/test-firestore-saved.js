...
async function run() {
  const q = query(collection(db, "saved_articles"), limit(2));
  const snap = await getDocs(q);
  console.log("Documents found in saved_articles:", snap.size);
  snap.forEach(doc => {
    console.log(doc.id, "Fields:", Object.keys(doc.data()), "Data:", doc.data());
  });
}
